...
`dsh-provider-qoder` 将你的 Qoder 订阅接入 DeepSeek Harness（DSH），支持流式文本、推理内容和工具调用，可选择国际版或中国区 Qoder 服务。
...
- 支持流式回答、推理内容、工具调用及文本工具结果的多轮交互。
- 在设置中查看账号信息、个人额度、组织资源包和重置时间。
...
获取模型使用的是**已保存的 PAT 和服务区域**。更换账号或区域后，请先保存，再重新获取模型。初始模型目录仅供候选参考，实际可用模型以账号查询结果为准。模型倍率及推理强度选项由 Qoder 提供，并非所有模型都有这些信息。
...
切换区域不会自动替换 PAT 或重新获取模型。保存对应区域的 PAT 后，重新获取可用模型、保存选择，并在对话中选择可用的 Qoder 模型。

### 请求是否会自动重试？

自动重试由 DSH 管理。需要重试时，运行中的 profile 应包含 `@deepseek-ai/dsh-llm-retry`。插件使用 DSH 默认重试策略报告空响应、限流、服务端、超时和传输错误；认证、无效请求、取消、额度及协议格式错误默认不重试。
